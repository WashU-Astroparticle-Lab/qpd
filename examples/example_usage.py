"""
Example usage of the QPD transmon simulator

This script demonstrates various ways to use the QPD class to analyze
QPD transmons.
"""
...
def example_1_basic_usage():
    ...
    """
    print("\n" + "=" * 70)
    print("Example 1: Basic QPD Transmon Simulation (WashU parameters)")
    print("=" * 70)
...
def example_3_from_capacitance():
    ...
    delta_hz = QPD.DELTA_AL / QPD.PLANCK_EV_S  # Hz
    ...
    print(f"Eⱼ = {qpd.e_j_hz / 1e9:.3f} GHz")
    print(f"Eᴄ = {qpd.e_c_hz / 1e9:.4f} GHz")
    print(f"Eⱼ/Eᴄ = {qpd.ej_ec_ratio:.2f}")
...
def example_4_custom_analysis():
    ...
    for i, u in enumerate(offset_charges):
        e_even = energies_even[i, 0] / qpd.PLANCK_EV_S / 1e9
        e_odd = energies_odd[i, 0] / qpd.PLANCK_EV_S / 1e9
        delta_e = energy_diff[i] / qpd.PLANCK_EV_S / 1e9
...
def main():
    ...
    print("\n" + "=" * 70)
    print(" QPD Transmon Simulator - Example Usage")
    print("=" * 70)
    print("\nThis script demonstrates various uses of the QPD class.")
    ...
    if choice == "1":
        qpd, figs = example_1_basic_usage()
    elif choice == "2":
        qpd, figs = example_2_serniak_parameters()
    elif choice == "3":
        qpd = example_3_from_capacitance()
    elif choice == "4":
        qpd = example_4_custom_analysis()
    elif choice == "5":
        fig = example_5_parameter_scan()
    else:
        # Run all examples
        print("\nRunning all examples...\n")
        qpd1, figs1 = example_1_basic_usage()
        qpd2, figs2 = example_2_serniak_parameters()
        qpd3 = example_3_from_capacitance()
        qpd4 = example_4_custom_analysis()
