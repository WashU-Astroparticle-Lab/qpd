...
Verification lives in `checks/`: `check_parity_reconstruction.py` is the reconstruction regression gate (55 checks), `check_reconstruction_benchmark.py` gates the surrogate-replay benchmark (48 checks, including the calibration test that its predicted efficiency matches the ensemble actually measured over independent traces), and `study_parity_reconstruction.py` runs the rate / noise / burst-crowding / device sweeps.
